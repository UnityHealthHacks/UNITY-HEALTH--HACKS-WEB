(() => {
  ...
  function renderResult(container, result, form) {
    ...
    if (result.limitText) {
      ...
      const strong = text('strong', 'Important limit: ');
      limit.append(strong, document.createTextNode(result.limitText));
      summary.append(limit);
    }
    const meta = text('p', `Route ID: ${result.routeId} · Taxonomy: ${result.taxonomyVersion}`, 'small');
    summary.append(meta);
  ...
  function resetGoal(form, container) {
    ...
    const first = form.querySelector('input[name="uhhGoal"]');
    first?.focus();

...
if(foodForm){foodForm.addEventListener('submit',e=>{
 ...
 terms.forEach(([t,d])=>{if(ing.includes(t))notes.push(`Ingredient note: ${t} is ${d}.`)}); if(lowerSodium)notes.push('Because you selected lower-sodium comparisons, compare sodium per serving across similar products and verify that serving sizes are comparable. A “reduced sodium” claim means less sodium than a reference product; it does not automatically mean the food is low in sodium.'); if(digestive)notes.push('Because you are tracking digestive comfort, record serving size, timing, and repeatable patterns rather than assuming one ingredient is the cause.'); if(barcode)notes.push(`Captured product code: ${barcode}. A live product database is not connected, so this code is not being used as verified product identity.`); if(!ing.trim())notes.push('Paste the full ingredient list for a more useful explanation.'); notes.push('This is educational information, not a diagnosis or treatment recommendation.');
